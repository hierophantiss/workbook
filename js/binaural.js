...
function buildBreathAudio() {
  ...
  var beatConfigs = {
    '4-2-6-1': { base: 160, beat: 10 },
    '4-7-8': { base: 160, beat: 6 },
    '5-5': { base: 160, beat: 10 },
    'sleep-delta': { base: 140, beat: 2 },
    'sleep-classical': { base: 140, beat: 2 } 
  };
  var config = beatConfigs[bCurrentPattern] || { base: 160, beat: 10 };
  ...
  // NO classical music loaded here, just the placeholder for now as requested
...
function startBreathAudio() {
  ...
  breathAudioOn = true;
  btn.style.color = 'rgba(168,213,220,1)';
  btn.style.borderColor = 'rgba(168,213,220,0.7)';
  btn.style.background = 'rgba(42,93,94,0.6)';
}
...
function stopBreathAudio() {
  ...
  breathAudioOn = false;
  btn.style.color = 'rgba(168,213,220,0.4)';
  btn.style.borderColor = 'rgba(168,213,220,0.2)';
...
function showBinauralModal() {
  ...
  var beatType = isSleep ? (isEl ? 'Delta' : 'Delta') : (isEl ? 'Alpha' : 'Alpha');
  var beatRange = isSleep ? ' (0.5-4 Hz)' : ' (8-12 Hz)';
  var beatDesc = isSleep 
    ? (isEl ? 'Τα binaural beats στέλνουν διαφορετική συχνότητα σε κάθε αυτί, δημιουργώντας ήχο στη ζώνη Delta' + beatRange + ' που ενισχύει τον βαθύ ύπνο.' : 'Binaural beats send a different frequency to each ear, creating a tone in the Delta range' + beatRange + ' that promotes deep sleep.')
    : (isEl ? 'Τα binaural beats στέλνουν διαφορετική συχνότητα σε κάθε αυτί, δημιουργώντας ήχο στη ζώνη Alpha' + beatRange + ' που ενισχύει την ήρεμη εγρήγορση.' : 'Binaural beats send a different frequency to each ear, creating a tone in the Alpha range' + beatRange + ' that promotes calm alertness.');
