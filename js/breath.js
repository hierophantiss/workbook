...
// ══════════════════════════════════════════════
// VOICE COACH — Web Speech API wrapper
// Soft female voice counts the breath: "inhale, 1, 2, 3, 4"
// Zero network cost, zero dependencies, works offline.
// ══════════════════════════════════════════════
var bVoiceEnabled = false;
var bVoiceSelected = { el: null, en: null };
var bVoiceLastSpokenKey = '';

function bVoiceAvailable() {
  return typeof window !== 'undefined' &&
         'speechSynthesis' in window &&
         typeof SpeechSynthesisUtterance !== 'undefined';
}

// Number words for counting (Greek & English, 1-10)
var B_NUM_WORDS = {
  el: ['', 'ένα', 'δύο', 'τρία', 'τέσσερα', 'πέντε', 'έξι', 'επτά', 'οκτώ', 'εννιά', 'δέκα'],
...
var B_PHASE_WORDS = {
  el: ['εισπνοή', 'κράτημα', 'εκπνοή', 'παύση'],
  en: ['inhale', 'hold', 'exhale', 'rest']
};

// Pick the best available voice for a language. Preference order:
// 1. Female voice matching the language
// 2. Any voice matching the language
// 3. null (let browser pick default)
function bPickVoice(langCode) {
  if (!bVoiceAvailable()) return null;
  var voices = speechSynthesis.getVoices();
  if (!voices || !voices.length) return null;
  var langPrefix = langCode.toLowerCase().split('-')[0]; // 'el' from 'el-GR'
  ...
  // Prefer female-sounding voices. Different platforms name them differently.
  var femaleHints = /female|woman|σοφία|sofia|melina|μελίνα|anna|alice|samantha|victoria|karen|google.*female|female.*google/i;
  var female = matching.find(function(v) { return femaleHints.test(v.name); });
  if (female) return female;
  // Avoid obvious male voices
  var maleHints = /male|man|daniel|alex|fred|oliver|arthur/i;
  var nonMale = matching.find(function(v) { return !maleHints.test(v.name); });
  return nonMale || matching[0];
}

// Cache best voice per language (voices load asynchronously on first page load)
function bRefreshVoices() {
  bVoiceSelected.el = bPickVoice('el-GR');
  bVoiceSelected.en = bPickVoice('en-US');
}

if (bVoiceAvailable()) {
  bRefreshVoices();
  // Voices often aren't loaded on first call — listen for the load event
  if (speechSynthesis.addEventListener) {
    speechSynthesis.addEventListener('voiceschanged', bRefreshVoices);
  } else {
    speechSynthesis.onvoiceschanged = bRefreshVoices;
  }
}

// Load persisted voice preference
try {
  var savedVoice = localStorage.getItem('breath_voice_enabled');
  if (savedVoice === '1') bVoiceEnabled = true;
} catch(e){}

// Speak a string with the appropriate language voice.
// `key` deduplicates — same key won't repeat back-to-back.
function bSpeak(text, key) {
  if (!bVoiceEnabled || !bVoiceAvailable() || !bRunning) return;
  if (key && key === bVoiceLastSpokenKey) return;
  if (key) bVoiceLastSpokenKey = key;
  try {
    // Cancel any queued speech — we want timely, non-overlapping cues
    speechSynthesis.cancel();
    var u = new SpeechSynthesisUtterance(text);
    u.lang = LANG === 'el' ? 'el-GR' : 'en-US';
    var v = bVoiceSelected[LANG];
    if (v) u.voice = v;
    u.rate = 0.92;   // slightly slower than default — calming
    u.pitch = 1.05;  // slightly higher — softer feel
    u.volume = 0.75; // not intrusive
    speechSynthesis.speak(u);
  } catch(e){}
}

// Called at the start of each phase — speaks the phase name
function bVoicePhaseStart(phaseIdx) {
  if (!bVoiceEnabled) return;
  var word = B_PHASE_WORDS[LANG][phaseIdx];
  bSpeak(word, 'phase_' + phaseIdx + '_' + Date.now());
}

// Called each second during a phase — speaks the current count
// Exception: if the phase is only 1 second long, we skip the count
// (the phase name itself is enough; counting "1" alone feels abrupt).
function bVoiceCount(phaseIdx, secondIndex, totalSecs) {
  if (!bVoiceEnabled) return;
  if (totalSecs < 2) return; // skip for 1-second phases
  var n = secondIndex + 1;
  var word = (B_NUM_WORDS[LANG][n] || String(n));
  bSpeak(word, 'count_' + phaseIdx + '_' + n);
}

// Toggle voice on/off. Called by the UI button.
function toggleBreathVoice() {
  ...
  if (!bVoiceEnabled && bVoiceAvailable()) speechSynthesis.cancel();
...
function bUpdateVoiceBtn() {
  ...
  if (!btn) return;
  btn.textContent = bVoiceEnabled ? '🔊' : '🔈';
  btn.style.color = bVoiceEnabled ? 'rgba(168,213,220,0.7)' : 'rgba(168,213,220,0.25)';
...
function bUpdatePhase(now) {
  ...
  if (currentCount !== bLastCountSec) {
    ...
    // Don't count on second 1 if we JUST spoke the phase name at phase-start
    if (currentCount > 1) bVoiceCount(bPhaseIdx, elapsedSecs, totalSecs);
  }
...
function bStart() {
  ...
  // Voice: announce the first phase (e.g. "εισπνοή") immediately
  bVoicePhaseStart(bPhaseIdx);
}
...
function bPause() {
  ...
  // Stop any queued/in-flight speech so it doesn't keep talking after pause
  if (bVoiceAvailable()) { try { speechSynthesis.cancel(); } catch(e){} }
...
function bReset() {
  ...
  // Stop any queued/in-flight speech
  if (bVoiceAvailable()) { try { speechSynthesis.cancel(); } catch(e){} }
