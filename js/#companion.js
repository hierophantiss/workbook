...
function updateFabPosition(fab) {
  ...
  if (id === 'home' || id === 'start') {
    ...
    if (companionData.fabPos) {
      fab.style.left = companionData.fabPos.x + 'px';
      fab.style.top = companionData.fabPos.y + 'px';
      fab.style.bottom = 'auto';
      fab.style.right = 'auto';
    } else {
      // Default floating: bottom-left as requested
      fab.style.left = '20px';
      fab.style.bottom = 'calc(var(--nav-h) + 20px)';
      fab.style.right = 'auto';
      fab.style.top = 'auto';
    }
  }
...
function makeDraggable(el) {
  ...
  function onMove(e) {
    ...
    // Bounds check
    x = Math.max(0, Math.min(window.innerWidth - 52, x));
...
function showCompanionBubble() {
  ...
  var smartActionsHtml = 
    '<button class="comp-smart-btn" onclick="hideCompanionBubble(); window.c2StartFlow(\'moodCheck\'); window.c2ShowSheet();">💭 ' + (lang === 'el' ? 'Πώς νιώθεις;' : 'How do you feel?') + '</button>' +
    '<button class="comp-smart-btn" onclick="hideCompanionBubble(); window.c2StartFlow(\'smartHub\'); window.c2ShowSheet();">🧭 ' + (lang === 'el' ? 'Τι να κάνω;' : 'What to do?') + '</button>';
