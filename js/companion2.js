...
function c2RenderSmartHub(step, lang) {
    ...
    var html = `<div class="c2-back-row"><button class="c2-back" onclick="window.c2ShowMainMenu()">←</button><span class="c2-back-title">${lang === 'el' ? 'Τι να κάνω;' : 'What to do?'}</span></div>`;
    html += '<div class="c2-options">';

    // CONTEXT A: Χρήστης διαβάζει κεφάλαιο
    if (activeScreen === 'screen-chapter') {
        var chNum = d.lastChapter || 1;
    ...
    // CONTEXT B: Χρήστης είναι στην αρχική ή αλλού
        ...
        if (d.lastChapter) {
            ...
            var chTitle = chs[d.lastChapter - 1] ? chs[d.lastChapter - 1].title : "...";
...
function c2RenderDynamicGreeting(step, lang) {
    ...
    var html = `
        <div class="c2-header">
            <div class="c2-greeting"><div class="c2-greeting-text">${greeting}</div></div>
            <button class="c2-close" onclick="window.c2HideSheet()">✕</button>
        </div>
...
function c2RenderFaqList(step, lang) {
    ...
    var html = `<div class="c2-back-row"><button class="c2-back" onclick="window.c2ShowMainMenu()">←</button><span class="c2-back-title">${title} (Κεφ. ${lastCh})</span></div>`;
...
function c2RenderFaqAnswer(step, lang) {
    ...
    var html = `<div class="c2-back-row"><button class="c2-back" onclick="window.c2RunStep('show_faq')">←</button><span class="c2-back-title">${item.q}</span></div>`;
...
function c2RenderConceptSelect(step, lang) {
    var concepts = window[step.source];
