...
function c2ShowSheet() {
    ...
    if (!sheet) return;
    c2EnsureState();
    // Μόνο αν δεν υπάρχει ήδη φορτωμένο flow, δείχνε το main menu
    if (!c2CurrentFlow) {
...
// Άνοιξε το sheet ΚΑΙ ξεκίνα flow — χωρίς race condition
function c2OpenFlow(flowId) {
    var sheet = document.getElementById('c2-sheet');
    if (!sheet) return;
    c2EnsureState();
    // Πρώτα κάνε visible το sheet ώστε το #c2-content να υπάρχει
    sheet.classList.add('visible');
    c2SheetVisible = true;
    var handle = sheet.querySelector('.c2-sheet-handle');
    if (handle) handle.setAttribute('aria-expanded', 'true');
    // Μετά ξεκίνα το flow
    window.c2StartFlow(flowId);
}
window.c2OpenFlow = c2OpenFlow;
...
function c2ShowMainMenu() {
    var lang = (typeof LANG !== 'undefined') ? LANG : 'el';
    var d = window.companionData;
    var lastChapter = (d && d.lastChapter) ? d.lastChapter : 1;
    var html = `
        <div class="c2-header">
        ...
        <div class="c2-menu">
            ...
            ${c2MenuActionBtn('📖', lang === 'el' ? 'Συνέχισε το διάβασμα' : 'Continue reading', 'window.c2HideSheet(); openChapter(' + lastChapter + ')')}
        </div>`;
    c2SetContent(html);
}
...
function c2MenuActionBtn(icon, title, action) {
    return `
